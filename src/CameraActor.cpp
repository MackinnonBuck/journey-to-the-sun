...
glm::mat4 CameraActor::computeMatrix()
{
...
void CameraActor::update(float deltaTime)
{
    ...
    m_transform =
        m_target.computeModelMatrix() *
        glm::rotate(glm::mat4(1.0f), glm::pi<float>() * 0.5f, glm::vec3(0.0f, 1.0f, 0.0f)) *
        glm::translate(glm::mat4(1.0f), glm::vec3(-18.0f, 3.0f, 0.0f)) *
        glm::rotate(glm::mat4(1.0f), -glm::pi<float>() * 0.5f, glm::vec3(0.0f, 1.0f, 0.0f));
        //* glm::mat4(glm::quat(0.5f, 0.0f, 0.0f, 0.f));
        //* glm::rotate(glm::mat4(1.0f), -glm::pi<float>() * 0.5f, glm::vec3(1.0f, 0.0f, 0.0f));

    m_position = -glm::vec3(m_transform[3]);
    m_transform = glm::inverse(m_transform);

	auto& editorState = getEditorState();
    editorState.cameraPosition = m_position;//glm::vec3(m_transform[3]);//getPosition();
