...
void CameraActor::update(float deltaTime)
{
	...
	m_transform =
		glm::translate(glm::mat4(1.0f), glm::vec3(11.601f, 4.9039f, -4.4362f)) *
		glm::mat4(glm::quat(0.214f, 0.377f, 0.783f, 0.445f)) *
		glm::rotate(glm::mat4(1.0f), glm::pi<float>() * 0.5f, glm::vec3(1.0f, 0.0f, 0.0f)) *
		m_target.computeMatrix();

	m_transform = glm::inverse(m_transform);

	auto& editorState = getEditorState();
	editorState.cameraPosition = -m_transform[3];//getPosition();
