...
void main()
{
	...
	diffuse_fact = 0.05 + diffuse_fact * 0.9;

	//vec3 half = normalize(camdir + lightdir);
	//float spec_fact = clamp(dot(half,frag_norm),0,1);

	vec4 tcol = vec4(1.0, 1.0, 1.0, 1.0) * 1.5;//texture(tex, vec2(vertex_tex.x,1-vertex_tex.y)); //the plane has reversed texture coordinates in y... blame the model!
	//color = tcol;
